...
class PPO():
    def __init__(self, state_dim, action_dim, batch_size):
        ...
        self.actor = Net_Actor(state_dim, action_dim, self.hidden_width)
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=self.lr)

        self.critic = Net_Critic(state_dim, self.hidden_width)
    ...
    def update(self, memory):
        ...
        for _ in range(self.K_epochs):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(self.batch_size)), self.mini_batch_size, False):
                ...
                ratios = a_prob_now / a_prob[index].detach()  # 这里的a_prob没有梯度
