...
class Actor_Beta(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_width):
        ...
        self.alpha_head = nn.Linear(hidden_width, action_dim)
        self.beta_head = nn.Linear(hidden_width, action_dim)

    def forward(self, s):
        s = torch.tanh(self.l1(s))
        s = torch.tanh(self.l2(s))
        # alpha and beta need to be larger than 1,so we use 'softplus' as the activation function and then plus 1
        alpha = F.softplus(self.alpha_head(s)) + 1.0
        beta = F.softplus(self.beta_head(s)) + 1.0
        return alpha, beta
...
class PPO():
    def __init__(self, state_dim, action_dim, batch_size):
        ...
        self.actor = Actor_Beta(self.state_dim, self.action_dim, self.hidden_width)
        self.optimizer1 = torch.optim.Adam(self.actor.parameters(), lr=self.lr)

        self.critic = Critic(self.state_dim, self.hidden_width)
        self.optimizer2 = torch.optim.Adam(self.critic.parameters(), lr=self.lr)
...
def evaluate_policy(env, agent, max_action):
    ...
    for _ in range(times):
        ...
        while not done:
            ...
            action = 2 * (a - 0.5) * max_action  # [0,1]->[-max,max]
...
if __name__ == '__main__':
    ...
    # Build a tensorboard
    writer = SummaryWriter(log_dir='runs/PPO_continuous/env_{}_Beta_number_{}_seed_{}'.format(env_name[env_index], number, seed))
    ...
    while total_steps < max_train_steps:
        ...
        while not done:
            ...
            if done and episode_steps != max_episode_steps:
                ...
                dw = False
            # take the 'action'，but store the original 'a'
            ...
            if (total_steps + 1) % evaluate_freq == 0:
                evaluate_num += 1
                evaluate_reward = evaluate_policy(env_evaluate, agent, max_action)
                evaluate_rewards.append(evaluate_reward)
                print("evaluate_num:{} \t evaluate_reward:{} \t".format(evaluate_num, evaluate_reward))
                writer.add_scalar('step_rewards_{}'.format(env_name[env_index]), evaluate_rewards[-1], global_step=total_steps)
                # Save the rewards
                if evaluate_num % 10 == 0:
                    np.save('./data_train/PPO_continuous_Beta_env_{}_number_{}_seed_{}.npy'.format(env_name[env_index], number, seed), np.array(evaluate_rewards))
